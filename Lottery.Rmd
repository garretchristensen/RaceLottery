...
server<- function(input, output) {
    ...
    output$value <- renderPrint({ input$num 
      ...
      w_output<-subset(women_winners, select=c("fullname", "State", "Age"))
      m_output<-subset(men_winners, select=c("fullname", "State", "Age"))
   ...
   #I can't figure out how to label tables, so just make the table itself
      ...
      #subset
      #dfnew5 <- subset(diamonds, select=c("color", "carat", "price"))
      m_output_wait<-subset(men_waiters, select=c("fullname", "State", "Age"))
      #Add ID column data$ID <- seq.int(nrow(data))
