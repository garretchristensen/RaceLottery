...
temp<-read.csv("./HL2020data_prelim.csv", stringsAsFactors = FALSE) #LOAD THE DATA
df<-as_tibble(temp)
...
Note that runners are allowed deferred entry from previous years due to pregnancy or new children or getting into Western States or Hardrock. We take these slots from their respective gender lotteries.
...
ui<-fluidPage(
  ...
  fluidRow(
  column(6, numericInput("num", label = h1("Enter the seed"), value = NULL)),
  column(6, numericInput("N", label = h1("Enter the race size"), value = 125))
  ),
  
  fluidRow(
  column(6, numericInput("w_defer", label = h1("Enter number of female deferrals"), value = 0)),
  column(6, numericInput("m_defer", label = h1("Enter number of male deferrals"), value = 5)) 
  ),             
